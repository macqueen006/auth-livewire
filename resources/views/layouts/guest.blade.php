...
<body id="auth-body" class="overflow-hidden relative w-screen h-screen" style="background-color:{{ config('appearance.background.color') }}">
@php
    $dyanicPageId = str_replace('/', '-', str_replace('.', '', Request::path()));
@endphp
<div x-data data-auth="{{ $dyanicPageId }}" class="relative w-full h-full" x-cloak>
    @if(config('appearance.background.image'))
        <img src="{{ config('appearance.background.image') }}" id="auth-background-image" class="object-cover absolute z-10 w-screen h-screen"  alt="img"/>
        <div id="auth-background-image-overlay" class="absolute inset-0 z-20 w-screen h-screen" style="background-color:{{ config('appearance.background.image_overlay_color') }}; opacity:{{ config('appearance.background.image_overlay_opacity') }};"></div>
    @endif

    @php
        $slotParentClasses = match(config('appearance.alignment.container')){
    ...
    @if(config('settings.enable_branding') && !app()->isLocal())
